...
function updateCoins() {
  ...
  const fertEl = document.getElementById('feed-count');
  if (fertEl) fertEl.textContent = Game.totalFeedCount();
}
...
function showRainEffect() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    item.title = isBug ? 'Bắt sâu (+coin)' : 'Nhặt con rơi';
...
function showApp() {
  ...
  let page = 'farm';
  try { page = sessionStorage.getItem('vx_page') || 'farm'; } catch (_) {}
  goToPage(page);
}
...
function openNycConfigModal() {
  ...
  const gi = getSelectedAgentFarm('nyc');
  const cfg = Game.getNycConfigForFarm ? Game.getNycConfigForFarm(gi) : base;
  renderAgentFarmToggles('nyc-farm-toggles', base.farmsEnabled, 'nyc');
  const sel = document.getElementById('nyc-animal-select');
  if (!sel) return;
  ...
  const opts = ['<option value="">— Chưa chọn con —</option>'];
  Object.keys(seeds).filter(id => (seeds[id] || 0) > 0).forEach(id => {
    const p = Game.getAnimal(id);
    if (!p) return;
    const val = id + '|normal';
    const selc = (cfg.animalId === id && cfg.seedKind === 'normal') ? 'selected' : '';
  ...
  Object.keys(stars).filter(id => (stars[id] || 0) > 0).forEach(id => {
    ...
    if (!p) return;
    const val = id + '|star';
    const selc = (cfg.animalId === id && cfg.seedKind === 'star') ? 'selected' : '';
  ...
  Object.keys(myths).filter(id => (myths[id] || 0) > 0).forEach(id => {
    ...
    if (!p) return;
    const val = id + '|myth';
    const selc = (cfg.animalId === id && cfg.seedKind === 'myth') ? 'selected' : '';
    opts.push(`<option value="${val}" ${selc}>${p.icon} ${p.name} ✨ · huyền thoại x${myths[id]}</option>`);
  });
  if (cfg.animalId) {
    const haveN = (seeds[cfg.animalId] || 0) > 0;
    const haveS = (stars[cfg.animalId] || 0) > 0;
    const haveM = (myths[cfg.animalId] || 0) > 0;
    if (cfg.seedKind === 'myth' && !haveM) {
      const p = Game.getAnimal(cfg.animalId);
      if (p) opts.push(`<option value="${cfg.animalId}|myth" selected>${p.icon} ${p.name} ✨ (hết con huyền thoại)</option>`);
    }
    if (cfg.seedKind === 'star' && !haveS) {
      const p = Game.getAnimal(cfg.animalId);
      if (p) opts.push(`<option value="${cfg.animalId}|star" selected>${p.icon} ${p.name} ⭐ (hết con sao)</option>`);
    }
    if (cfg.seedKind === 'normal' && !haveN) {
      const p = Game.getAnimal(cfg.animalId);
      if (p) opts.push(`<option value="${cfg.animalId}|normal" selected>${p.icon} ${p.name} (hết con thường)</option>`);
    }
  }
  ...
  mountPillDropdown(sel, { prefix: 'Động vật:', block: true });
}
...
function bindNycConfigUI() {
  ...
  document.getElementById('btn-save-nyc-config')?.addEventListener('click', async () => {
    const raw = document.getElementById('nyc-animal-select')?.value || '';
    let animalId = null, seedKind = 'normal';
    if (raw && raw.includes('|')) {
      const parts = raw.split('|');
      animalId = parts[0] || null;
    ...
    const gi = getSelectedAgentFarm('nyc');
    const enEl = document.getElementById('nyc-farm-enabled');
    const applyAll = !!document.getElementById('nyc-apply-all-farms')?.checked;
    const baseGe = Object.assign({}, (Game.getNycConfig().farmsEnabled || {}));
    if (enEl) baseGe[String(gi)] = !!enEl.checked;
    
    const nFarms = typeof Game.getFarmCount === 'function' ? Game.getFarmCount() : 1;
    for (let i = 0; i < nFarms; i++) {
      if (baseGe[String(i)] === undefined && baseGe[i] === undefined) baseGe[String(i)] = true;
    }
    const payload = {
      farmIndex: gi,
      farmEnabled: enEl ? !!enEl.checked : true,
      farmsEnabled: baseGe,
    ...
    if (res.ok && applyAll && animalId) {
      ...
      res = { ok: true, msg: 'Đã lưu NYC · áp dụng con cho tất cả trại đang bật' };
    }
    if (res.ok) {
      await savePlayer();
      showToast(res.msg, 'success');
...
window._agentFarmTab = window._agentFarmTab || { fairy: 0, nyc: 0 };

function renderAgentFarmToggles(hostId, farmsEnabled, kind) {
  const host = document.getElementById(hostId);
  if (!host) return;
  if (typeof Game.ensureFarms === 'function') Game.ensureFarms();
  const n = typeof Game.getFarmCount === 'function' ? Game.getFarmCount() : 1;
  const ge = farmsEnabled && typeof farmsEnabled === 'object' ? farmsEnabled : {};
  const k = kind || 'fairy';
  let sel = Number(window._agentFarmTab[k]) || 0;
  if (sel < 0 || sel >= n) sel = 0;
  window._agentFarmTab[k] = sel;
  let html = '<div class="agent-farm-tabs">';
  for (let i = 0; i < n; i++) {
  ...
  if (n >= 1) {
    ...
    html += `<label class="farm-toggle-row agent-farm-enable">
      <span class="farm-toggle-label"><i class="fa-solid fa-power-off"></i> Bật trên Trại ${sel + 1}</span>
      <input type="checkbox" class="farm-toggle-switch" id="${k}-farm-enabled" data-farm="${sel}" ${on ? 'checked' : ''} />
    </label>
    <p class="bulk-hint">Đang cấu hình <strong>Trại ${sel + 1}</strong> — mỗi trại có cấu hình riêng.</p>`;
    if (k === 'nyc') {
      html += `<label class="farm-toggle-row" style="margin-top:6px">
        <span class="farm-toggle-label"><i class="fa-solid fa-copy"></i> Áp dụng con này cho <strong>tất cả trại đang bật</strong> khi Lưu</span>
        <input type="checkbox" id="nyc-apply-all-farms" checked />
      </label>`;
    }
  } else {
  ...
  host.querySelectorAll('.agent-farm-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      const i = parseInt(btn.dataset.farm, 10) || 0;
      const kk = btn.dataset.kind || k;
      window._agentFarmTab[kk] = i;
...
function readAgentFarmToggles(hostId) {
  const host = document.getElementById(hostId);
  const ge = {};
  if (!host) return ge;
  
  host.querySelectorAll('.farm-toggle-switch').forEach(inp => {
    const i = inp.dataset.farm;
    ge[i] = !!inp.checked;
  });
  return ge;
}

function getSelectedAgentFarm(kind) {
  const n = typeof Game.getFarmCount === 'function' ? Game.getFarmCount() : 1;
  let sel = Number(window._agentFarmTab[kind]) || 0;
  if (sel < 0 || sel >= n) sel = 0;
  return sel;
}
...
function openFairyConfigModal() {
  ...
  const gi = getSelectedAgentFarm('fairy');
  const cfg = Game.getFairyConfigForFarm ? Game.getFairyConfigForFarm(gi) : base;
  renderAgentFarmToggles('fairy-farm-toggles', base.farmsEnabled, 'fairy');
  ...
  if (cfg.useFeed) { if (fOn) fOn.checked = true; }
...
function bindFairyConfigUI() {
  ...
  document.getElementById('btn-save-fairy-config')?.addEventListener('click', async () => {
    const gi = getSelectedAgentFarm('fairy');
    const enEl = document.getElementById('fairy-farm-enabled');
    const baseGe = Object.assign({}, (Game.getFairyConfig().farmsEnabled || {}));
    ...
    const res = Game.setFairyConfig({
      ...
      useFeed: document.querySelector('input[name="fairy-fert-on"]:checked')?.value !== '0',
        ...
        try {
          if (typeof syncPlayerOnEnter === 'function') {
            ...
            if (typeof renderFarm === 'function') {
              const gp = document.getElementById('page-farm');
              if (gp && gp.classList.contains('active')) renderFarm();
            }
          }
...
async function renderRank() {
  ...
  try {
    ...
    const labels = { raised: 'đã nuôi', harvested: 'thu hoạch', coins: 'xu', collection: 'sưu tầm' };
...
async function renderFriends() {
  ...
  try {
    ...
    list.querySelectorAll('.btn-visit-friend').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        openVisitFarm(btn.dataset.uid, btn.dataset.name || nameMap[btn.dataset.uid] || 'Bạn');
      });
    });
...
async function openVisitFarm(friendUid, friendName) {
  ...
  try {
    ...
    if (!snap.exists()) {
      grid.innerHTML = '<p class="empty-state">Bạn này chưa đồng bộ trại công khai.<br>Họ cần vào game một lần (cập nhật Rules nếu lỗi).</p>';
      return;
    }
    const data = snap.val();
    const pens = Array.isArray(data.pens) ? data.pens : Object.values(data.pens || {});
    if (meta) {
      ...
      meta.textContent = `Lv.${data.level || 1} · ${pens.length} ô · Cập nhật: ${updated}`;
    }
    if (!pens.length) {
      grid.innerHTML = '<p class="empty-state">Trại trống.</p>';
      return;
    }
    grid.innerHTML = '';
    pens.forEach((pen, i) => {
      const div = document.createElement('div');
      div.className = 'pen visit-pen';
      if (!pen.animalId) {
        div.classList.add('empty');
        div.innerHTML = `<div class="pen-icon">🟫</div><div class="pen-name">Trống</div>`;
      } else {
        ...
        const water = pen.waterCount > 0 ? `<span class="pen-badge-water">💧${pen.waterCount > 1 ? pen.waterCount : ''}</span>` : '';
        const fert = pen.feedId ? `<span class="pen-badge-fert">🧪</span>` : '';
        div.innerHTML = `
          <div class="pen-badges"><span class="pen-badge-left">${water}</span><span class="pen-badge-right">${fert}</span></div>
          <div class="pen-icon">${stage.icon || (animal && animal.icon) || '🌱'}</div>
          <div class="pen-name">${animal ? animal.name : pen.animalId}</div>
          <div class="pen-status">${ready ? '✨ Sẵn sàng' : stage.label + ' · ' + progress + '%'}</div>
          ${!ready ? `<div class="pen-progress"><div class="pen-progress-bar" style="width:${progress}%"></div></div>` : ''}
        `;
      }
...
document.getElementById('btn-save-profile')?.addEventListener('click', async () => {
  ...
  updateHelperBadge();
  try {
    document.querySelectorAll('.farm-decor-fairy').forEach(el => {
      el.style.display = Game.showFairyDecor && Game.showFairyDecor() ? '' : 'none';
    });
    document.querySelectorAll('.farm-decor-nyc').forEach(el => {
      el.style.display = Game.showNycDecor && Game.showNycDecor() ? '' : 'none';
    });
    document.querySelectorAll('.farm-decor-helper').forEach(el => {
...
function closeAllPillMenus(except) {
  document.querySelectorAll('.pill-dd.open').forEach(dd => {
    ...
    dd._portalMenu = null;
    document.querySelector('#modal-pen .modal-content')?.classList.remove('dropdown-open');
...
if (!window.__pillDdDocBound) {
  ...
  window.addEventListener('scroll', (e) => {
    ...
    if (t && t.closest && t.closest('#modal-pen .modal-content')) {
      document.querySelectorAll('.pill-dd.open').forEach(dd => {
...
function mountPillDropdown(select, opts = {}) {
  ...
  trigger.addEventListener('click', (e) => {
    ...
    if (!isOpen) {
      ...
      document.querySelector('#modal-pen .modal-content')?.classList.add('dropdown-open');
      positionMenu();
    }
  });
...
function mountAllPillDropdowns(root) {
  ...
  const map = {
    ...
    'bank-term': 'Kỳ hạn:',
    'nyc-animal-select': 'Động vật:',
    'fairy-fert-id': 'Loại phân:',
    'merge-animal': 'Con ghép:',
  ...
  scope.querySelectorAll('select').forEach(sel => {
    ...
    const block = sel.classList.contains('boost-pen-sel') ||
      ['nyc-animal-select', 'fairy-fert-id', 'merge-animal', 'merge-protect', 'empty-upgrade-select', 'pen-upgrade-select', 'p-type'].includes(id) ||
      sel.dataset.pillBlock === '1';
    if (sel.classList.contains('boost-pen-sel')) {
...
function renderFarmSwitcher() {
  ...
  let menuItems = '';
  for (let i = 0; i < n; i++) {
    const pens = (currentPlayer.farms && currentPlayer.farms[i]) || [];
    const count = pens.length || 0;
    const isActive = i === active;
    menuItems += `<button type="button" class="pill-dd-item ${isActive ? 'active' : ''}" data-farm="${i}" role="option" aria-selected="${isActive}">
      <span class="pill-dd-item-text"><i class="fa-solid fa-house-chimney-window"></i> Trại ${i + 1} <small>${count}/${maxP}</small></span>
      ${PILL_CHECK_SVG}
    </button>`;
  }
  ...
  let hintHtml = '';
  if (n === 1) {
    const count = (currentPlayer.pens || []).length;
    if (count < maxP) {
      hintHtml = `<span class="farm-switch-hint">Đủ ${maxP} ô sẽ mở Trại 2</span>`;
    }
  }
  ...
  host.innerHTML = `
    <div class="pill-dd" id="farm-dropdown">
      ...
      <div class="pill-dd-menu" id="farm-dd-menu" role="listbox" hidden>
        ${menuItems}
      </div>
    </div>
    ${hintHtml}`;

  const dropdown = host.querySelector('#farm-dropdown');
  const trigger = host.querySelector('#farm-dd-trigger');
  const menu = host.querySelector('#farm-dd-menu');
  ...
  menu.querySelectorAll('.pill-dd-item').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const idx = parseInt(btn.dataset.farm, 10);
      closeMenu();
      if (isNaN(idx)) return;
      if (idx === Game.getActiveFarmIndex()) return;
      // Chặn spam click khi đang chuyển
      if (window._farmSwitchBusy) return;
      window._farmSwitchBusy = true;
      try {
        const res = Game.switchFarm(idx);
        showToast(res.msg, res.ok ? 'success' : 'error');
        if (res.ok) {
          renderFarm();
          updateCoins();
...
function renderFarm() {
  ...
  pens.forEach((pen, i) => {
    ...
    const speedM = (typeof Game.getPenSpeedMult === 'function') ? Game.getPenSpeedMult(pen) : (pen.specialMult || 1);
    if (pen && speedM > 1) {
      div.classList.add('special-pen');
      const badge = document.createElement('span');
      badge.className = 'pen-special-badge';
      const tempOn = pen.specialMultUntil && pen.specialMultUntil > Date.now();
      badge.textContent = 'x' + speedM + (tempOn ? '⏱' : '');
      badge.title = tempOn
        ? ('Tạm: x' + (pen.specialMultTemp || speedM) + ' còn ' + Game.formatTime(Math.ceil((pen.specialMultUntil - Date.now()) / 1000)))
    ...
    if (!pen.animalId) {
      ...
      if (!animal) {
        ...
        if (isMyth) div.classList.add('pen-myth');
        else if (isStar) div.classList.add('pen-star');
        else div.classList.add('pen-normal');
        const starBadge = isMyth
          ? `<span class="pen-badge-myth" title="Con huyền thoại">✨</span>`
          : (isStar ? `<span class="pen-badge-star" title="Con sao">⭐</span>` : '');
        ...
        const timerHtml = !ready
          ? `<div class="pen-timer" data-role="timer"${remain > 0 && remain <= 10 ? ' hidden' : ''}><i class="fa-regular fa-clock"></i> ${Game.formatTime(remain)}</div>`
          : '';
        div.innerHTML = `
        ...
        `;
        div.addEventListener('click', () => openPenModal(i));
      }
    }
    grid.appendChild(div);
  });
  ...
  const agents = document.getElementById('farm-agents');
  const agentHost = agents || grid;
  if (agents) agents.innerHTML = '';
  else grid.querySelectorAll('.farm-decor').forEach(el => el.remove());
  if (Game.showFairyDecor && Game.showFairyDecor()) {
    const emoji = (Game.getFairyEmoji && Game.getFairyEmoji()) || '🧚';
    for (let k = 0; k < 3; k++) {
      const f = document.createElement('div');
      f.className = 'farm-decor farm-decor-fairy farm-roamer';
      f.textContent = emoji;
  ...
  if (Game.showNycDecor && Game.showNycDecor()) {
    ...
    for (let k = 0; k < 3; k++) {
      ...
      p.className = 'farm-decor farm-decor-nyc farm-roamer';
      p.textContent = emoji;
  ...
  if (Game.showHelperDecor && Game.showHelperDecor()) {
    ...
    for (let k = 0; k < 3; k++) {
      ...
      h.className = 'farm-decor farm-decor-helper farm-roamer';
      h.textContent = emoji;
...
function openEmptyPenModal(plotId) {
  ...
  openAnimalModal(plotId);
  const list = document.getElementById('animal-buy-list');
  if (!list) return;
  ...
  if (!higher.length) {
    ...
    html += '<div class="pen-upgrade-row"><select id="empty-upgrade-select">';
    higher.forEach(x => {
      const cost = Features.getPenUpgradeCost(curMult, x.mult);
  ...
  document.getElementById('btn-empty-upgrade')?.addEventListener('click', async (e) => {
    ...
    const res = await Features.upgradePen(plotId, tgt);
    showToast(res.msg, res.ok ? 'success' : 'error');
    if (res.ok) {
      updateCoins();
      closeModals();
...
function openAnimalModal(plotId) {
  ...
  if (ids.length === 0) {
    ...
    ids.sort((a, b) => {
      const pa = Game.getAnimal(a), pb = Game.getAnimal(b);
      return (pa?.type || '').localeCompare(pb?.type || '');
    });
    ids.forEach(id => {
      const animal = Game.getAnimal(id);
      if (!animal) return;
      ...
      const maxAnimal = Math.min(have, empty);
      ...
      opt.innerHTML = `
        ...
        <div class="info" style="flex:1;min-width:0">
          ...
          <div class="grow-time">${Game.formatTime(animal.raiseTime)}</div>
        </div>
    ...
    list.querySelectorAll('.btn-animal-n').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const row = btn.closest('.seed-option');
        const input = row?.querySelector('.animal-qty-input');
        let n = parseInt(input?.value, 10) || 1;
        const max = parseInt(input?.max, 10) || 1;
        n = Math.max(1, Math.min(n, max));
        const kind = btn.dataset.kind || null;
        const res = await Game.raiseMultiple(btn.dataset.id, n, kind);
        showToast(res.msg, res.ok ? 'success' : 'error');
        closeModals();
        renderFarm();
        updateCoins();
  ...
  document.getElementById('modal-animal').classList.add('show');
  if (typeof enhanceQtyInputs === 'function') enhanceQtyInputs(document.getElementById('animal-buy-list'));
}
...
function openPenModal(plotId) {
  ...
  document.getElementById('pen-detail').innerHTML = `
  ...
  const higher = tiers.filter(x => x.mult > curMult);
  let upgradeHtml = '<div class="pen-upgrade-box"><h4 class="pen-upgrade-title"><i class="fa-solid fa-bolt"></i> <span>Nâng cấp vĩnh viễn</span> · hiện tại <span class="pen-mult-badge">x' + curMult + '</span></h4>';
  if (!higher.length) {
    upgradeHtml += '<p class="bulk-hint">Đã đạt mức tối đa x50.</p></div>';
  } else {
    upgradeHtml += '<div class="pen-upgrade-row"><select id="pen-upgrade-select">';
    higher.forEach(x => {
      const cost = Features.getPenUpgradeCost(curMult, x.mult);
      upgradeHtml += '<option value="' + x.mult + '">x' + x.mult + ' — ' + Number(cost).toLocaleString() + '🪙</option>';
    });
    upgradeHtml += '</select><button type="button" class="btn btn-warning btn-sm" id="btn-upgrade-pen"><i class="fa-solid fa-arrow-up"></i> Nâng cấp</button></div></div>';
  }
  document.getElementById('pen-detail').insertAdjacentHTML('beforeend', upgradeHtml);
  document.getElementById('btn-upgrade-pen')?.addEventListener('click', async () => {
    const tgt = parseFloat(document.getElementById('pen-upgrade-select')?.value);
    const res = await Features.upgradePen(plotId, tgt);
    showToast(res.msg, res.ok ? 'success' : 'error');
    if (res.ok) {
      updateCoins();
  ...
  mountPillDropdown(document.getElementById('pen-upgrade-select'), { prefix: 'Nâng cấp:', block: true });

  document.getElementById('modal-pen').classList.add('show');
}


function softUpdatePenModal() {
  const modal = document.getElementById('modal-pen');
  if (!modal || !modal.classList.contains('show')) return;
  if (selectedPenId == null || !currentPlayer) return;
  const pen = currentPlayer.pens[selectedPenId];
  ...
  const remain = Game.getRemainingSeconds(pen);
  const stage = Game.getStage(pen);
  const detail = document.getElementById('pen-detail');
  if (!detail) return;

  const stageEl = detail.querySelector('[data-role="pen-stage"]');
  if (stageEl) stageEl.textContent = ready ? '✨ Sẵn sàng thu hoạch' : stage.label + ' (' + progress + '%)';

  const remainEl = detail.querySelector('[data-role="pen-remain"]');
  if (remainEl) remainEl.textContent = ready ? '0s' : Game.formatTime(remain);

  const pctEl = detail.querySelector('[data-role="pen-pct"]');
  ...
  if (waterEl) {
    ...
    waterEl.classList.toggle('pen-boost-off', !waterDisp.active);
  }
  const fertEl = detail.querySelector('[data-role="pen-fert"]');
  if (fertEl) {
...
function openBulkModal(action) {
  ...
  if (action === 'fert') {
    const ferts = (currentPlayer?.inventory?.feeds) || {};
    const ids = Object.keys(ferts).filter(id => ferts[id] > 0);
    if (!ids.length) {
      list.innerHTML = '<p class="empty-state">Không còn cám cho ăn trong kho.</p>';
    ...
    const sorted = ids.map(id => Game.getFeed(id)).filter(Boolean)
...
(function bindFarmToolsMenu() {
  const wrap = document.getElementById('farm-tools-dd');
  const btn = document.getElementById('btn-tools-menu');
  if (!wrap || !btn) return;
  btn.addEventListener('click', (e) => {
    e.stopPropagation();
    document.getElementById('farm-support-dd')?.classList.remove('open');
...
(function bindFarmSupportMenu() {
  const wrap = document.getElementById('farm-support-dd');
...
function openFertModal(plotId) {
  ...
  if (ids.length === 0) {
    ...
    ids.forEach(id => {
      ...
      opt.addEventListener('click', async () => {
        const res = await Game.applyFeed(plotId, id);
        showToast(res.msg, res.ok ? 'success' : 'error');
        closeModals();
        renderFarm();
        updateCoins();
  ...
  document.getElementById('modal-pen').classList.remove('show');
...
function getShopAnimalsFiltered() {
  ...
  animals = animals.slice().sort((a, b) => {
    const la = Game.isAnimalLimited(a) && Game.isAnimalAvailable(a) ? 0 : 1;
    const lb = Game.isAnimalLimited(b) && Game.isAnimalAvailable(b) ? 0 : 1;
    return la - lb;
  });
  return animals;
}
...
function renderShop() {
  ...
  if (currentShopTab === 'chuong') {
    ...
    try {
      if (typeof Features !== 'undefined' && Features.getPenUpgradeCost) {
        const farms = (currentPlayer.farms && currentPlayer.farms.length)
          ? currentPlayer.farms
          : [currentPlayer.pens || []];
        farms.forEach(g => {
          ...
          arr.forEach(pl => {
            ...
            if (base >= 50) return;
            const c = Features.getPenUpgradeCost(base, 50);
            if (c != null && c >= 0) {
              upgradeAllNeed += c;
              upgradeAllCount++;
            }
    ...
    grid.innerHTML = `
      <div class="shop-pen-row">
        <div class="shop-card shop-pen-card">
    ...
    document.getElementById('btn-upgrade-all-x50')?.addEventListener('click', async () => {
      ...
      updateCoins();
      renderShop();
      if (res.ok && typeof renderFarm === 'function') renderFarm();
    });
    return;
  }
  ...
  if (currentShopTab === 'tangtoc') {
    ...
    hint.innerHTML = '⏱ Gói tạm (1–30 ngày): chọn ô để áp dụng. ⚡ Nâng <strong>vĩnh viễn</strong>: mở chuồng → Nâng cấp.';
    ...
    packs.forEach(pack => {
      ...
      card.innerHTML = `
        ...
        <select class="boost-pen-sel" data-id="${pack.id}" data-pill-prefix="Ô:" data-pill-block="1">${plotOpts || '<option value="">Chưa có ô</option>'}</select>
    ...
    grid.querySelectorAll('.btn-buy-temp-boost').forEach(btn => {
      btn.addEventListener('click', async () => {
        const sel = grid.querySelector('.boost-pen-sel[data-id="' + btn.dataset.id + '"]');
        const penId = sel ? sel.value : '0';
        const res = await Features.buyTempPenBoost(btn.dataset.id, penId);
        showToast(res.msg, res.ok ? 'success' : 'error');
        updateCoins();
        if (res.ok) { renderFarm(); renderShop(); }
      });
    });
  ...
  if (currentShopTab === 'cam') {
    ...
    DEFAULT_FEEDS.forEach(fert => {
      const have = (currentPlayer?.inventory?.feeds?.[fert.id]) || 0;
      const card = document.createElement('div');
      card.className = 'shop-card';
      card.innerHTML = `
        <div class="shop-icon">${fert.icon}</div>
        <div class="shop-name">${fert.name}</div>
        <span class="shop-type">Thức ăn</span>
    ...
    document.querySelectorAll('.btn-buy-fert').forEach(btn => {
      const buy = async (qty) => {
        const fert = Game.getFeed && Game.getFeed(btn.dataset.id);
  ...
  if (currentShopTab === 'pet') {
    ...
    grid.querySelectorAll('.btn-buy-pet').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        updateCoins();
        renderShop();
        if (typeof renderFarmPets === 'function') renderFarmPets();
      });
    });
    return;
  }

  const animals = getShopAnimalsFiltered();
  const totalPages = Math.max(1, Math.ceil(animals.length / SHOP_PAGE_SIZE));
  if (shopPage >= totalPages) shopPage = totalPages - 1;
  const slice = animals.slice(shopPage * SHOP_PAGE_SIZE, (shopPage + 1) * SHOP_PAGE_SIZE);

  const countEl = document.getElementById('shop-count');
  if (countEl) countEl.textContent = `${animals.length} loại · trang ${shopPage + 1}/${totalPages}`;
  ...
  slice.forEach(animal => {
    const have = seeds[animal.id] || 0;
    const limited = Game.isAnimalLimited(animal);
    const available = Game.isAnimalAvailable(animal);
    const card = document.createElement('div');
    card.className = 'shop-card' + (limited ? ' shop-card-limited' : '') + (!available ? ' shop-card-locked' : '');
    const badge = limited
      ? `<span class="badge-limited" title="${Game.getLimitedEventLabel(animal)}">Limited</span>`
      : '';
    ...
    card.innerHTML = `
      ...
      <span class="shop-type">${TYPE_LABELS[animal.type] || animal.type}</span>
      <div class="shop-desc">${animal.desc || ''}</div>
      ${eventLine}
      <div class="shop-meta">
        <span><i class="fa-regular fa-clock"></i> ${animal.raiseTime}s</span>
...
function renderInventory() {
  ...
  const seedsEl = document.getElementById('inv-animals');
  const stars = (currentPlayer.inventory && currentPlayer.inventory.animalsStar) || {};
  const myths = (currentPlayer.inventory && currentPlayer.inventory.animalsMyth) || {};
    ...
    html += '<div class="inv-grid">' + ids.map(id => {
      ...
      return `
        <div class="inv-item${kind === 'myth' ? ' inv-item-myth' : ''}">
          <div class="icon">${animal.icon}${tag}</div>
          <div class="name${nameLong}">${nameStr}</div>
          <div class="qty${qtyLong}">${qtyStr}</div>
          <div class="actions">
  ...
  if (!normalIds.length && !starIds.length && !mythIds.length) {
    seedsEl.innerHTML = '<p class="empty-state">Chưa có động vật. Hãy mua ở Cửa hàng!</p>';
  } else {
    let sHtml = '';
    sHtml += renderSeedBag(
    ...
    seedsEl.querySelectorAll('.btn-sell-seed').forEach(btn => {
      ...
      const sell = async (qty) => {
        let n = qty;
        if (n === 'all') {
          if (kind === 'myth') n = (currentPlayer.inventory.animalsMyth && currentPlayer.inventory.animalsMyth[id]) || 0;
          else if (kind === 'star') n = (currentPlayer.inventory.animalsStar && currentPlayer.inventory.animalsStar[id]) || 0;
          else n = (currentPlayer.inventory.animals && currentPlayer.inventory.animals[id]) || 0;
        } else {
          n = Math.max(1, Math.floor(Number(n) || 1));
        }
        ...
        bindPressHold(btn, {
          ...
          onHold: () => {
            const have = kind === 'myth'
              ? ((currentPlayer.inventory.animalsMyth && currentPlayer.inventory.animalsMyth[id]) || 0)
              : (kind === 'star'
  ...
  if (fertIds.length === 0) {
    ...
    fertEl.querySelectorAll('.btn-sell-fert').forEach(btn => {
      ...
      const sell = async (qty) => {
        let n = qty;
        if (n === 'all') n = (currentPlayer.inventory.feeds && currentPlayer.inventory.feeds[id]) || 0;
        else n = Math.max(1, Math.floor(Number(n) || 1));
        const res = await Game.sellFeed(id, n);
        showToast(res.msg, res.ok ? 'success' : 'error');
        updateCoins();
        renderInventory();
      };
      if (btn.dataset.qty === 'all') {
        ...
        bindPressHold(btn, {
          onClick: () => sell(1),
          onHold: () => {
            const have = (currentPlayer.inventory.feeds && currentPlayer.inventory.feeds[id]) || 0;
            openQtyPickModal({
  ...
  const renderHarvestBag = (bag, kind, title, emptyMsg, priceFn) => {
    ...
    html += `<div class="inv-grid">` + ids.map(id => {
      ...
      if (!animal) return '';
      const qty = bag[id];
      const unit = priceFn(animal);
      return `
        <div class="inv-item">
          <div class="icon">${animal.icon}</div>
  ...
  if (mergeEl) {
    if (typeof window._mergeSel === 'undefined') {
      window._mergeSel = { animalId: null, protectId: null, mythAnimalId: null, mythProtectId: null };
    }
    const starsBag = (currentPlayer.inventory && currentPlayer.inventory.animalsStar) || {};
    ...
    // --- Ghép sao ---
    if (!mergeable.length) {
      html += '<div class="merge-box"><p class="empty-state">Cần ≥ 2 con <strong>thường</strong> cùng loại để ghép thành con ⭐ (+50% sản lượng & giá bán).</p></div>';
    } else {
      let selAnimal = window._mergeSel.animalId;
      if (!selAnimal || !mergeable.includes(selAnimal)) selAnimal = mergeable[0];
      let selProt = window._mergeSel.protectId || '';
      if (selProt && !pids.includes(selProt)) selProt = '';
      const opts = mergeable.map(id => {
        const pl = Game.getAnimal(id);
        if (!pl) return '';
        const sel = id === selAnimal ? ' selected' : '';
    ...
    if (!mergeableStar.length) {
      html += '<div class="merge-box merge-box-myth"><p class="empty-state">Cần ≥ 2 con <strong>sao ⭐</strong> cùng loại để ghép <strong>huyền thoại ✨</strong> (x2 sản lượng, aura huyền bí).</p></div>';
    } else {
      let selM = window._mergeSel.mythAnimalId;
      ...
      html += `
        <div class="merge-box merge-box-myth">
          <p class="merge-lead">🌌 Ghép <strong>2 con sao</strong> → <strong>1 con huyền thoại ✨</strong></p>
          <p class="merge-sub">Cùng tỉ lệ bùa như ghép sao (bùa 100% = chắc chắn thành công). Thất bại mất 1 con sao. Con huyền thoại toát aura tím, x2 sản lượng.</p>
          <label>Chọn con sao</label>
          <select id="merge-myth-animal">${optsM}</select>
          <label>Bùa bảo hộ (tuỳ chọn)</label>
    ...
    const animalSel = document.getElementById('merge-animal');
    const protSel = document.getElementById('merge-protect');
    animalSel?.addEventListener('change', () => { window._mergeSel.animalId = animalSel.value || null; });
    protSel?.addEventListener('change', () => { window._mergeSel.protectId = protSel.value || null; });
    if (animalSel) window._mergeSel.animalId = animalSel.value || null;
    if (protSel) window._mergeSel.protectId = protSel.value || null;

    const doMerge = async (times) => {
      const pid = animalSel?.value;
      ...
      if (times === 'all' || times === 'max' || times === Infinity) {
        ...
        n = Math.floor(have / 2);
        if (n < 1) { showToast('Cần ít nhất 2 con thường cùng loại!', 'error'); return; }
      } else {
    ...
    const mythAnimalSel = document.getElementById('merge-myth-animal');
    const mythProtSel = document.getElementById('merge-myth-protect');
    mythAnimalSel?.addEventListener('change', () => { window._mergeSel.mythAnimalId = mythAnimalSel.value || null; });
    mythProtSel?.addEventListener('change', () => { window._mergeSel.mythProtectId = mythProtSel.value || null; });
    if (mythAnimalSel) window._mergeSel.mythAnimalId = mythAnimalSel.value || null;
    if (mythProtSel) window._mergeSel.mythProtectId = mythProtSel.value || null;

    const doMergeMyth = async (times) => {
      const pid = mythAnimalSel?.value;
      const pr = mythProtSel?.value || null;
      window._mergeSel.mythAnimalId = pid || null;
      window._mergeSel.mythProtectId = pr || null;
      if (!pid) { showToast('Chọn con sao!', 'error'); return; }
      let n;
      if (times === 'all' || times === 'max' || times === Infinity) {
        const have = (currentPlayer.inventory.animalsStar && currentPlayer.inventory.animalsStar[pid]) || 0;
        n = Math.floor(have / 2);
        if (n < 1) { showToast('Cần ít nhất 2 con sao cùng loại!', 'error'); return; }
      } else {
        n = Math.max(1, Math.floor(Number(times) || 1));
      }
...
function renderStats() {
  ...
  document.getElementById('stats-grid').innerHTML = `
    ...
    <div class="stat-card">
      <div class="value">${pens.filter(p => p.animalId).length}/${pens.length}</div>
      <div class="label"><i class="fa-solid fa-border-all"></i> Ô đang nuôi</div>
    </div>
    <div class="stat-card">
  ...
  if (albumEl) {
    ...
    const totalPages = Math.max(1, Math.ceil(animals.length / pageSize));
    if (window._albumPage >= totalPages) window._albumPage = totalPages - 1;
    const page = window._albumPage;
    const slice = animals.slice(page * pageSize, (page + 1) * pageSize);
...
const TREE_TIERS = [
  { min: 1, max: 99, class: 'tier-tree-1', title: 'Nông Dân Tập Sự', icon: 'fa-egg', desc: 'Mới bắt đầu chăn nuôi', glow: 'rgba(133, 83, 53, 0.4)' },
  { min: 100, max: 499, class: 'tier-tree-2', title: 'Chủ Chuồng Nhỏ', icon: 'fa-dove', desc: 'Nuôi gà vịt thành thạo', glow: 'rgba(74, 222, 128, 0.45)' },
  { min: 500, max: 999, class: 'tier-tree-3', title: 'Trại Chủ Tập Sự', icon: 'fa-paw', desc: 'Mở rộng đàn gia súc', glow: 'rgba(56, 189, 248, 0.5)' },
  { min: 1000, max: 1999, class: 'tier-tree-4', title: 'Chủ Trại Vàng', icon: 'fa-cow', desc: 'Đàn bò dê phát triển', glow: 'rgba(250, 204, 21, 0.55)' },
  { min: 2000, max: 2999, class: 'tier-tree-5', title: 'Cao Thủ Chăn Nuôi', icon: 'fa-horse', desc: 'Ngựa bò đầy trại', glow: 'rgba(244, 114, 182, 0.6)' },
  { min: 3000, max: 4499, class: 'tier-tree-6', title: 'Đại Gia Nông Trại', icon: 'fa-dragon', desc: 'Trại lớn bậc nhất vùng', glow: 'rgba(0, 242, 254, 0.7)' },
  { min: 4500, max: 5999, class: 'tier-tree-7', title: 'Chúa Tể Đồng Cỏ', icon: 'fa-sun', desc: 'Thống trị đồng cỏ bao la', glow: 'rgba(245, 158, 11, 0.75)' },
  { min: 6000, max: 7499, class: 'tier-tree-8', title: 'Huyền Thoại Chăn Nuôi', icon: 'fa-shield-cat', desc: 'Tên tuổi vang xa', glow: 'rgba(192, 132, 252, 0.85)' },
  { min: 7500, max: 8999, class: 'tier-tree-9', title: 'Thần Nông Trại', icon: 'fa-hat-cowboy', desc: 'Bậc thầy chăn nuôi', glow: 'rgba(244, 63, 94, 0.95)' },
  { min: 9000, max: 9499, class: 'tier-tree-10', title: 'Hoàng Đế Gia Súc', icon: 'fa-crown', desc: 'Đế chế chăn nuôi', glow: 'rgba(147, 197, 253, 0.9)' },
  { min: 9500, max: 9999, class: 'tier-tree-11', title: 'Huyền Thoại Bất Tử', icon: 'fa-fire', desc: 'Huyền thoại bất diệt', glow: 'rgba(225, 29, 72, 0.95)' },
  { min: 10000, max: 10000, class: 'tier-tree-12', title: 'Tối Thượng Mục Đồng', icon: 'fa-trophy', desc: 'Đỉnh cao chăn nuôi', glow: 'rgba(255, 255, 255, 0.95)' }
];
...
function updateTreeLevelUI(val) {
  ...
  if (icon) icon.className = 'fa-solid ' + (tier.icon || 'fa-youngling') + ' tree-icon';
  if (lvlNum) lvlNum.textContent = level;
  if (pillIcon) pillIcon.className = 'fa-solid ' + (tier.icon || 'fa-youngling');
...
function softUpdateFarm() {
  if (!currentPlayer) return;
  tickFarmCare({ render: false });
  softUpdateFarmUI();
}
function softUpdateFarmUI() {
  if (!currentPlayer) return;
  // Đang chạy NYC care — chỉ cập nhật nút đếm ngược, không đụng grid trại đang xem
  if (typeof Game !== 'undefined' && Game._nycBusy) {
    if (typeof updateHarvestCountdownButton === 'function') updateHarvestCountdownButton();
    return;
  }
  const farmPage = document.getElementById('page-farm');
  if (farmPage && !farmPage.classList.contains('active')) return;
  const pens = Array.isArray(currentPlayer.pens) ? currentPlayer.pens : Object.values(currentPlayer.pens || {});
  let needFull = false;
  pens.forEach((pen, i) => {
...
/** Mỗi trại sắp chín (1–10s, chưa isReady) → 1 nút riêng cạnh Hỗ trợ.
 *  Con đã chín (isReady) bị bỏ qua — tránh kẹt mãi ở "1s" do Math.ceil. */
function updateHarvestCountdownButton() {
  ...
  if (Array.isArray(currentPlayer.farms) && currentPlayer.farms.length) {
    farms = currentPlayer.farms;
  } else if (currentPlayer.farms && typeof currentPlayer.farms === 'object') {
    const keys = Object.keys(currentPlayer.farms).sort((a, b) => Number(a) - Number(b));
    farms = keys.map(k => currentPlayer.farms[k]);
  } else {
    farms = [Array.isArray(currentPlayer.pens) ? currentPlayer.pens : Object.values(currentPlayer.pens || {})];
  }

  const entries = [];
  farms.forEach((pens, gi) => {
    if (!pens) return;
    ...
    list.forEach((pen) => {
      ...
      if (typeof Game.isReady === 'function' && Game.isReady(pen)) return;
      if (typeof Game.getProgress === 'function' && Game.getProgress(pen) >= 100) return;
      let remain = Game.getRemainingSeconds ? Game.getRemainingSeconds(pen) : 0;
  ...
  // Không có trại trong cửa sổ 10s → xóa hết nút
  if (!entries.length) {
    host.innerHTML = '';
    host.hidden = true;
    return;
  ...
  // Đồng bộ DOM: mỗi trại 1 nút, cập nhật text hoặc tạo mới, xóa nút thừa
  const want = new Set(entries.map(e => String(e.index)));
  host.querySelectorAll('[data-farm-cd]').forEach(btn => {
    if (!want.has(btn.dataset.farmCd)) btn.remove();
  });

  entries.forEach((e) => {
    let btn = host.querySelector(`[data-farm-cd="${e.index}"]`);
    if (!btn) {
      btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'btn btn-boost-countdown';
      btn.dataset.farmCd = String(e.index);
      btn.innerHTML = `<i class="fa-solid fa-hourglass-half"></i><span class="cd-text"></span>`;
      host.appendChild(btn);
    }
    const textEl = btn.querySelector('.cd-text');
    const label = `Trại ${e.index}: ${e.sec}s`;
...
function forceBackgroundCare(reason) {
  ...
  try {
    ...
    const changed = tickFarmCare({ render: reason === 'login' || reason === 'visible' });
    if (changed) {
...
function fillMarketItemSelect() {
  ...
  const bagKey = kind === 'seed' ? 'animals'
  ...
  host.innerHTML = deps.map(d => {
    ...
    return `<div class="bank-item" data-dep-id="${d.id}">
      <div class="bank-item-main">
        ...
        <div class="bank-timer pen-timer" data-role="bank-remain">${matured ? '<i class="fa-solid fa-circle-check"></i> Đáo hạn — nhận ' + fullPayout.toLocaleString() + '🪙' : '<i class="fa-regular fa-clock"></i> ' + Game.formatTime(remain)}</div>
      </div>
...
(function setupFarmHorizontalWheel() {
  const grid = document.getElementById('farm-grid');
  if (!grid) return;
...
function renderKitchen() {
  ...
  const haveIng = (animalId, tier) => {
    if (tier === 'myth') return harvestMyth[animalId] || 0;
    if (tier === 'star') return harvestStar[animalId] || 0;
    return harvest[animalId] || 0;
  };
  const canCookTier = (r, tier) => (r.ingredients || []).every(ing => haveIng(ing.animalId, tier) >= (ing.qty || 1));
  ...
  if (cookEl && (cookEl.style.display !== 'none')) {
    ...
    cookEl.innerHTML = `
      ...
      const ings = (r.ingredients || []).map(ing => {
        ...
        const n = haveIng(ing.animalId, 'normal');
        const s = haveIng(ing.animalId, 'star');
        const m = haveIng(ing.animalId, 'myth');
        const okAny = n >= need || s >= need || m >= need;
        return `<span class="kitchen-ing ${okAny ? 'ok' : 'no'}">${p ? p.icon : '❓'}${p ? p.name : ing.animalId} ×${need}
...
function renderFarmPets() {
  const agents = document.getElementById('farm-agents');
  const host = agents || document.getElementById('farm-grid');
  if (!host || !currentPlayer) return;
  host.querySelectorAll('.farm-pet').forEach(el => el.remove());
  ...
  activeIds.forEach((id, i) => {
    ...
    el.className = 'farm-pet farm-roamer';
    el.dataset.petId = id;
...
const _origRenderFarm = typeof renderFarm === 'function' ? renderFarm : null;
if (_origRenderFarm && !renderFarm._petsHooked) {
  window.renderFarm = function () {
    _origRenderFarm.apply(this, arguments);
    renderFarmPets();
  };
  renderFarm._petsHooked = true;
}
...
function fillHelperItemSelect() {
  ...
  sel.innerHTML = '';
  if (kind === 'seed') {
    (Game.getAnimals() || []).forEach(pl => {
      if (!pl || !pl.id) return;
      const o = document.createElement('option');
      o.value = pl.id;
      o.textContent = `${pl.icon || ''} ${pl.name}`.trim() + ` (${(pl.buyPrice || 0).toLocaleString()}🪙)`;
      sel.appendChild(o);
    });
  } else if (kind === 'fert') {
    (Game.getFeeds() || []).forEach(f => {
      const o = document.createElement('option');
      o.value = f.id;
