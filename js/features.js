...
const Features = {
  ...
  getPenUpgradeCost(currentMult, targetMult) {
  ...
  async upgradePen(plotId, targetMult) {
    ...
    if (!pen) return { ok: false, msg: 'Ô không tồn tại!' };
    ...
    const cost = this.getPenUpgradeCost(cur, tgt);
    ...
    pen.specialName = 'Ô x' + tgt + ' (vĩnh viễn)';
    pen.specialId = 'upgrade-perm-' + tgt;
  ...
  async upgradeAllPensTo(targetMult = 50) {
    ...
    const farms = Array.isArray(currentPlayer.farms) && currentPlayer.farms.length
      ? currentPlayer.farms
      : [currentPlayer.pens];
    const jobs = [];
    let totalCost = 0;
    farms.forEach((g, gi) => {
      const pens = Array.isArray(g) ? g : Object.values(g || {});
      if (!Array.isArray(g)) farms[gi] = pens;
      pens.forEach((pen, pi) => {
        if (!pen) return;
        const base = Number(pen.specialMultPermanent) || 1;
        if (base >= tgt) return;
        const cost = this.getPenUpgradeCost(base, tgt);
        if (cost == null || cost < 0) return;
        jobs.push({ gi, pi, pen, base, cost });
        totalCost += cost;
      });
    });
    ...
    if (typeof Game !== 'undefined' && Game.syncActiveFarm) {
      try { Game.syncActiveFarm(); } catch (_) {}
    } else if (Array.isArray(currentPlayer.farms) && typeof currentPlayer.activeFarm === 'number') {
      currentPlayer.pens = currentPlayer.farms[currentPlayer.activeFarm] || currentPlayer.pens;
    }
  ...
  async buyTempPenBoost(packId, penId) {
    if (!currentPlayer) return { ok: false, msg: 'Chưa đăng nhập!' };
    ...
    const active = pen.specialMultUntil && pen.specialMultUntil > now;
    if (active && Number(pen.specialMultTemp) === pack.mult) {
      pen.specialMultUntil = pen.specialMultUntil + addMs;
    } else {
      pen.specialMultTemp = pack.mult;
      pen.specialMultUntil = now + addMs;
    }
    pen.specialMult = Math.max(Number(pen.specialMultPermanent) || 1, pack.mult);
    pen.specialName = 'x' + pack.mult + ' · ' + pack.days + ' ngày';
    currentPlayer.stats = currentPlayer.stats || {};
  ...
  async redeemGiftCode(code) {
    ...
    const pensAdd = Math.max(0, parseInt(g.pens, 10) || 0);
    ...
    if (pensAdd > 0) {
      if (!Array.isArray(currentPlayer.pens)) currentPlayer.pens = Object.values(currentPlayer.pens || {});
    ...
    if (fertAdd > 0) {
      ...
      currentPlayer.inventory.feeds['cam-thuong'] =
        (currentPlayer.inventory.feeds['cam-thuong'] || 0) + fertAdd;
      parts.push('+' + fertAdd + ' cám thường');
    }
    if (fairyDays > 0) {
    ...
    if (g.animals && typeof g.animals === 'object') {
      if (!currentPlayer.inventory.animals) currentPlayer.inventory.animals = {};
      Object.keys(g.animals).forEach(pid => {
        const n = parseInt(g.animals[pid], 10) || 0;
        if (n > 0) currentPlayer.inventory.animals[pid] = (currentPlayer.inventory.animals[pid] || 0) + n;
      });
    }
  ...
  _marketBagKey(kind) {
    if (kind === 'seed') return 'animals';
    if (kind === 'animalStar') return 'animalsStar';
  ...
  async listMarketItem(kind, itemId, qty, priceEach) {
    ...
    const animal = typeof Game !== 'undefined' ? Game.getAnimal(itemId) : null;
    ...
    try {
      await db.ref('market/' + id).set({
        ...
        kind,
        itemId,
        itemName: animal ? animal.name : itemId,
        itemIcon: animal ? animal.icon : '🌱',
        qty,
        priceEach,
        total: qty * priceEach,
        createdAt: Date.now()
  ...
  async cancelMarketItem(listingId) {
    ...
    // 2. Thử xóa bằng transaction (an toàn với race mua)
    let deleted = false;
    try {
      const tx = await ref.transaction(cur => {
      ...
      deleted = !!(tx && tx.committed);
    } catch (e) {
      console.warn('cancelMarketItem tx', e);
      // Không return ngay — sẽ fallback remove bên dưới
    }

    // 3. Fallback: nếu transaction không commit, thử remove() trực tiếp
    //    (một số rule / SDK cũ đôi khi transaction return null bị abort dù quyền vẫn đủ)
    if (!deleted) {
      try {
        const recheck = await ref.once('value');
        if (!recheck.exists()) {
          // Đã bị người khác mua trong lúc đó → không hoàn kho
          return { ok: false, msg: 'Tin đã được mua hoặc không còn!' };
        }
        const cur = recheck.val();
        if (!cur || cur.sellerUid !== currentUser.uid) {
          return { ok: false, msg: 'Tin đã được mua hoặc không còn!' };
        }
        await ref.remove();
        deleted = true;
        ...
        console.warn('cancelMarketItem fallback remove', e);
    ...
    // 4. Hoàn kho đúng túi gốc
    const bagKey = this._marketBagKey(removed.kind) || (removed.kind === 'seed' ? 'animals' : 'harvest');
    if (!currentPlayer.inventory[bagKey]) currentPlayer.inventory[bagKey] = {};
    ...
    try {
      ...
      // Lưu kho fail → cố gắng đăng lại tin để không mất hàng
      try {
