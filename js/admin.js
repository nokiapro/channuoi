...
document.querySelectorAll('.side-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (btn.dataset.section === 'animals') renderAnimalsTable();
...
async function renderDashboard() {
  ...
  userList.forEach(u => {
    ...
    totalAnimaled += (u.stats && u.stats.raised) || 0;
  ...
  document.getElementById('admin-stats').innerHTML = `
    ...
    <div class="admin-stat"><div class="value">${currentSettings.penCount}</div><div class="label">Chuồng mặc định</div></div>
  `;
...
function renderAnimalsTable() {
  ...
  const pager = document.getElementById('animals-pager');
  const pageSize = 50;
  const q = (document.getElementById('admin-animal-search')?.value || '').trim().toLowerCase();
  let all = currentAnimals || [];
  if (q) {
    all = all.filter(p =>
  ...
  if (window._adminAnimalsPage == null) window._adminAnimalsPage = 0;
  const totalPages = Math.max(1, Math.ceil(all.length / pageSize));
  if (window._adminAnimalsPage >= totalPages) window._adminAnimalsPage = totalPages - 1;
  if (window._adminAnimalsPage < 0) window._adminAnimalsPage = 0;
  const page = window._adminAnimalsPage;
  const slice = all.slice(page * pageSize, (page + 1) * pageSize);
  ...
  document.querySelectorAll('#animals-table .btn-edit').forEach(btn => {
    btn.addEventListener('click', () => openAnimalForm(btn.dataset.id));
  });
  document.querySelectorAll('#animals-table .btn-delete').forEach(btn => {
  ...
  pager.querySelectorAll('[data-ux-page]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const p = parseInt(btn.getAttribute('data-ux-page'), 10);
      if (isNaN(p) || p === page) return;
      window._adminAnimalsPage = p;
      renderAnimalsTable();
    });
  });
}

document.getElementById('btn-add-animal').addEventListener('click', () => openAnimalForm(null));
...
function openAnimalForm(id) {
  ...
  if (id) {
    ...
    document.getElementById('p-type').value = animal.type;
    document.getElementById('p-seed-price').value = animal.buyPrice;
    document.getElementById('p-grow-time').value = animal.raiseTime;
    document.getElementById('p-yield').value = animal.yield;
    document.getElementById('p-sell-price').value = animal.sellPrice;
    document.getElementById('p-desc').value = animal.desc || '';
    document.getElementById('p-limited').checked = !!animal.limited;
    document.getElementById('p-months').value = Array.isArray(animal.availableMonths) ? animal.availableMonths.join(',') : '';
  } else {
    document.getElementById('form-title').textContent = 'Thêm con mới';
    document.getElementById('p-limited').checked = false;
    document.getElementById('p-months').value = '';
  }
  document.getElementById('modal-animal-form').classList.add('show');
}
...
document.getElementById('animal-form').addEventListener('submit', async (e) => {
  ...
  const availableMonths = monthsRaw
    ...
    : [];
  const data = {
    id: id || ('animal-' + Date.now()),
    icon: (document.getElementById('p-icon').value || '').trim().slice(0, 10),
    name: document.getElementById('p-name').value.trim(),
    type: document.getElementById('p-type').value,
    buyPrice: parseInt(document.getElementById('p-seed-price').value),
    raiseTime: parseInt(document.getElementById('p-grow-time').value),
  ...
  if (id) {
    const idx = currentAnimals.findIndex(p => p.id === id);
    if (idx >= 0) currentAnimals[idx] = data;
  } else {
...
async function renderUsers() {
  ...
  tbody.innerHTML = uids.map(uid => {
    ...
    return `
      <tr style="${banned ? 'opacity:0.65' : ''}${unlim ? ';background:rgba(34,197,94,0.08)' : ''}">
        ...
        <td>${(u.stats && u.stats.raised) || 0}</td>
        <td>${(u.stats && u.stats.harvested) || 0}</td>
        <td class="actions">
          <button class="btn btn-primary btn-add-coins" data-uid="${uid}">+ Tiền</button>
          <button class="btn btn-secondary btn-add-pens" data-uid="${uid}">+ Ô thường</button>
  ...
  document.querySelectorAll('.btn-add-pens').forEach(btn => {
    btn.addEventListener('click', async () => {
      const n = parseInt(prompt('Số chuồng thường thêm:', '1'), 10);
      if (!n || n < 1) return;
      const ref = db.ref('users/' + btn.dataset.uid);
      try {
        const tx = await ref.transaction(u => {
          if (!u) return u;
          if (!Array.isArray(u.pens)) u.pens = Object.values(u.pens || {});
          for (let i = 0; i < n; i++) {
            u.pens.push({
...
function renderSettings() {
  document.getElementById('set-pens').value = currentSettings.penCount || 12;
...
document.getElementById('btn-save-settings').addEventListener('click', async () => {
  currentSettings.penCount = parseInt(document.getElementById('set-pens').value) || 12;
...
async function renderGiftCodes() {
  ...
  tbody.innerHTML = keys.map(code => {
    ...
    if (g.pens) rewards.push('+' + g.pens + ' ô');
...
document.getElementById('btn-create-gc')?.addEventListener('click', async () => {
  ...
  const pens = parseInt(document.getElementById('gc-pens')?.value, 10) || 0;
...
document.getElementById('admin-animal-search')?.addEventListener('input', () => {
  window._adminAnimalsPage = 0;
  renderAnimalsTable();
});
...
document.getElementById('btn-send-announce')?.addEventListener('click', async () => {
  ...
  const title = (document.getElementById('mail-title')?.value || '').trim() || 'Thư từ Trại Xanh';
  if (!text) {
    ...
    Object.keys(users).forEach(uid => {
      ...
      if (Number(b.day) === d && Number(b.month) === m) {
        ...
        updates['mail/' + uid + '/' + mid] = {
          ...
          body: `Chúc ${(u.displayName || 'bạn')} sinh nhật vui vẻ từ Trại Xanh!`,
          from: 'Trại Xanh',
          type: 'birthday',
          at: Date.now(),
          read: false
...
document.getElementById('btn-reset-animals').addEventListener('click', async () => {
...
(function () {
  ...
  const _open = typeof openAnimalForm === 'function' ? openAnimalForm : null;
  if (_open) {
